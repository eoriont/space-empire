from strategies.strategy_util import get_possible_spots, is_in_bounds, get_spaces, get_other_player


class AggressiveStrategy:
    buy_destroyer = True
    __name__ = 'AggressiveStrategy'
    ...
    def decide_ship_movement(self, unit_index, game_state):
        ...
        other_player = get_other_player(game_state, self.player_index)
        distances = [dist(other_player["home_coords"], pos)
    ...
    def decide_removal(self, game_state):
        return next(i for i, x in enumerate(game_state["players"][self.player_index]["units"]) if x['type'] != 'Colony')
